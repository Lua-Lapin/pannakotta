...
function TimelineSection() {
  ...
  const days = RELAY_EVENT.schedule.reduce((acc, slot, i) => {
    if (i === 0 || slot.day !== RELAY_EVENT.schedule[i - 1].day) {
      acc.push({ day: slot.day, slots: [] })
    }
        ...
        {members.map((p, i) => (
          <Fragment key={p.order}>
            <div className="flex flex-col items-center gap-1">
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-pc-border">
                <img
                  src={p.img}
                  alt={p.name}
                  loading="lazy"
